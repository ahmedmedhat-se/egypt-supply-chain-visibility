...
import { FaShip, FaUsers, FaBuilding, FaExclamationTriangle, FaCheckCircle } from 'react-icons/fa';
...
export const DashboardPage = () => {
  ...
  const displayName = roleLabel.charAt(0).toUpperCase() + roleLabel.slice(1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[#94A3B8] dark:text-[#94A3B8] mt-1">
          {user.organizationName} — {displayName}
        </p>
      </div>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {stat.sub && (
                    <p className="text-xs text-[#2D9B6E]">{stat.sub}</p>
                  )}
                </div>
              </div>
            </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Team Members (for org-scoped) / Users by Role (for super_admin) */}
        <Card variant="bordered">
          <h2 className="text-lg font-semibold text-[#0A2E4A] dark:text-white mb-4">
            {isSuperAdmin ? 'Users by Role' : 'Team Members'}
          </h2>
      ...
      <Card variant="bordered" className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <FaExclamationTriangle
            className={`w-5 h-5 ${stats.criticalAlerts > 0 ? 'text-[#DC2626]' : 'text-[#2D9B6E]'}`}
          />
